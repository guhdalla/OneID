...
public abstract class ImageTransform {
    ...
    public static void createMapAndImgPushView(Map<String, Usuario> mapeamento, List<Usuario> usuarios) throws IOException {
        for (Usuario usuario : usuarios) {
            if (usuario.getFotoPerfil() != null) {
                String namespaceURI = usuario.getUsername().trim();
                File temp = File.createTempFile(namespaceURI, SUFFIX_IMG, new File(PATH_LOCAL_IMG));
                writesImageInTemp(usuario, temp);
                mapeamento.put(temp.getName(), usuario);
            }else{
